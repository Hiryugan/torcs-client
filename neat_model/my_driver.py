    ...
    def drive(self, carstate: State):
        ...
        if self.use_supervised:
            ...
            if self.parser.merge_accel_brake == 'sum1111':
                ...
                t_features_numpy_final[0] = np.sum(carstate.distances_from_edge[:4])
                t_features_numpy_final[1] = np.sum(carstate.distances_from_edge[4:8])
                t_features_numpy_final[2] = np.sum(carstate.distances_from_edge[8:11])
                t_features_numpy_final[3] = np.sum(carstate.distances_from_edge[11:-4])
                t_features_numpy_final[4] = np.sum(carstate.distances_from_edge[-4:])
